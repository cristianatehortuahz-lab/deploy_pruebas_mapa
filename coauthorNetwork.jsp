...
String rebuildCache(RDFService rdf, String basePath) throws Exception {
    ...
    try {
        Writer fw = new OutputStreamWriter(new FileOutputStream(basePath), "UTF-8");
        fw.write(jsonOutput);
        fw.close();
    } catch (Exception ecx) {
        System.err.println("Error guardando baseData.json: " + ecx.getMessage());
    }
    return jsonOutput;
}
%>
...
try {
    ...
    String basePath = application.getRealPath("/mapadeCoauthor/baseData.json");
    File cacheFile = new File(basePath);
    if (cacheFile.exists() && cacheFile.length() > 10) {
        ...
        cacheIn.close();
        final String rebuildPath = basePath;
        final RDFService rebuildRdf = rdf;
        synchronized(REBUILD_LOCK) {
            if (!isBuilding) {
                isBuilding = true;
                new Thread(new Runnable() { public void run() {
                    try { rebuildCache(rebuildRdf, rebuildPath); }
                    catch(Exception e) { System.err.println("Rebuild async error: " + e.getMessage()); }
                    finally { synchronized(REBUILD_LOCK) { isBuilding = false; } }
                }}).start();
            }
        }
    } else {
        String jsonOutput = rebuildCache(rdf, basePath);
